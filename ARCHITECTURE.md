...
│   │   ├── docs/                  # Live documentation viewer (Nous Hermes style)
│   │   │   ├── layout.tsx         # Left sidebar with doc categories layout
│   │   │   └── [[...slug]]/
│   │   │       └── page.tsx       # Dynamic Markdown viewer parsing docs/*.md files
│   │   └── api/
...
<RootLayout>                          ← ThemeProvider, fonts, Toaster
  └─ <GangNiagaAIOS>                 ← Main SPA shell
       ...
       │    ├─ Logo + Brand + Search Input
       │    ├─ NavGroup("Core")
       │    │    ├─ NavItem(Dashboard)
       │    │    ├─ NavItem(Business Plans) [Collapsible]
       │    │    │    ├─ SubItem(Idea Canvas)
       │    │    │    ├─ SubItem(Plan Review)
       │    │    │    └─ SubItem(Pitch Deck)
       │    │    └─ NavItem(Financials) [Collapsible]
       │    │         ├─ SubItem(Plan vs Actuals)
       │    │         └─ SubItem(Reports)
       │    ├─ NavGroup("Automation")
       │    │    └─ NavItem(Agent Console) [Collapsible]
       │    │         ├─ SubItem(Research Agent)
       │    │         ├─ SubItem(Workflows)
       │    │         └─ SubItem(Memory Engine)
       │    ├─ NavGroup("Integrations")
       │    │    └─ NavItem(Channel Connect) [Collapsible] (formerly OpenClaw)
       │    │         └─ SubItem(MCP Servers)
       │    └─ NavGroup("System")
       │         ├─ NavItem(Settings)
       │         └─ NavItem(Documentation) [Opens /docs in new tab]
       │
       ├─ <Header>                    ← Top bar
       │    ├─ SearchTrigger(⌘K)
       │    ├─ NotificationBell
...
│
├── Live Documentation (2 routes) ─────────────────────────────
│   ├── GET  /docs                 → Main Documentation Portal (Nous Hermes theme)
│   └── GET  /docs/[[...slug]]     → Catch-all dynamic Markdown loader
```
